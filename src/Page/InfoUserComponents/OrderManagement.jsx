...
import { requestCancelOrder, requestGetPaymentByUserId, requestReturnBook } from '../../config/request';
...
const OrderManagement = () => {
                ...
                <div className="flex gap-2">
                    ...
                    {record.status === 'delivered' && (
                        <Button
                            ...
                            className="flex items-center hover:bg-purple-600"
                        >
                            Trả sách
                        </Button>
                    )}
    ...
    const handleReturnSubmit = async () => {
        if (!returnReason.trim()) {
            message.error('Vui lòng nhập lý do trả sách');
            return;
        }

        try {
            await requestReturnBook({
            ...
            fetchOrders();
            message.success('Yêu cầu trả sách đã được gửi thành công');
        } catch (error) {
            message.error('Gửi yêu cầu trả sách thất bại');
        }
    };
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
            ...
            <Tabs defaultActiveKey="all">
                ...
                >
                    <Table
                        columns={columns}
                        dataSource={orders.filter((order) => order.status === 'returned')}
                        rowKey="id"
                        pagination={false}
                        className="mt-4"
                    />
                    ...
                    <div>
                        <Descriptions bordered column={2} size="small" className="mb-6">
                            ...
                            {selectedOrder.status === 'returned' && (
                                <>
                                    <Descriptions.Item label="Lý do trả sách" span={2}>
                                        {selectedOrder.returnReason || 'Không có lý do'}
                                    </Descriptions.Item>
                                    <Descriptions.Item label="Ngày yêu cầu trả sách" span={2}>
                                        {selectedOrder.returnRequestDate ? dayjs(selectedOrder.returnRequestDate).format('DD/MM/YYYY HH:mm') : 'Không có'}
                                    </Descriptions.Item>
                                </>
                            )}
                        </Descriptions>
            ...
            >
                <div className="mb-4">
                    <p className="text-gray-600 mb-2">
                        Đơn hàng: {orderToReturn?._id}
                    </p>
                    <p className="text-gray-600 mb-4">
                        Vui lòng nhập lý do trả sách:
                    </p>
                    <textarea
                        value={returnReason}
                        onChange={(e) => setReturnReason(e.target.value)}
                        placeholder="Nhập lý do trả sách..."
